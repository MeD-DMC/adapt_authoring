  ...
  callback(null, {
    ...
    parse: function (options, cb) {
      ...
      for (let i = 0; i < statements.length; i++) {
        ...
        if (_.isEmpty(statement.condition) || _.isEqual(statement.condition, options)) {
          ...
          if (effect === 'allow') {
            allowed = true; // this may be overridden by further statements
          }
        }
      }
      cb(null, allowed);
...
exports = module.exports = {
  ...
  hasPermission: function (userId, action, resource, callback, options) {
    //console.log(options);
    this.getUserPolicyStatements(userId, function (error, statements) {
      ...
      new PolicyStatementParser(action, resource, statements, function (error, parser) {
        ...
        return parser.parse(options, callback);
      });
    });
  },
