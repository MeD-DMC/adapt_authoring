...
exports = module.exports = {
  ...
  handleUserCoursesOnTransfer: function (fromUserId, toUserId, done) {
    ...
    var CourseContent = require(plugins.content.course.fullPath);
    var courseContent = new CourseContent();
    courseContent.retrieve({ createdBy: fromUserId }, {}, (err, results) => {
      async.eachSeries(results, (model, cb) => {
        this.transferSingleCourse(courseContent, model, toUserId, cb);
      }, done);
    });
  },
   ...
   *
   * @param {object} courseContent - courseContent
   * @param {object} model - course model
   * @param {string} toUserId - must match a user ID in db
   * @param {function} callback - function of the form function (error)
   */
  transferSingleCourse: function (courseContent, model, toUserId, callback) {
    // For a full description of behaviour see https://github.com/adaptlearning/adapt_authoring/pull/2277
    const sharedWithList = model._shareWithUsers;
    const isPartiallyShared = sharedWithList && sharedWithList.length;
    ...
    courseContent.update({ _id: model._id }, update, callback);
  },
  ...
  transferAllCourses: function (fromUser, toUserId, callback) {
    // confirm the user exists and is there is only one of them
    this.retrieveUser(fromUser, function (error, result) {
      if (error) {
        callback(error);
      } else if (result) {
        database.getDatabase(function (error, db) {
          ...
          async.parallel([
            function (cb) { this.handleUserCoursesOnTransfer(fromUser._id, toUserId, cb); }.bind(this)
          ], callback);
  ...
  validatePassword: function(password) {
    ...
    if (!/[~`!#$%\^&*+=\-\[\]\\';,/{}|\\":<>\?]/g.test(password)) errors = ['missingspecialchars', ...errors];

    if (!/\d/.test(password)) errors = ['missingnumber', ...errors];

    if (!/[A-Z]/.test(password)) errors = ['missinguppercase', ...errors];

    if (!password || password.length < 8) errors = ['tooshort', ...errors];
  ...
  init: function (app) {
    ...
    rest.post('/user/resetpassword', function (req, res, next) {
      ...
      if (delta.password) {
        if (self.validatePassword(delta.password).length > 0) {
          return res.status(500).json('Make your password stronger with more capital letters, more numbers and special characters');
        }
    ...
    rest.post('/transfer/course/:course_id/to_user/:to_user_id', function (req, res, next) {
      ...
      helpers.hasCoursePermission('update', currentUser._id, currentUser.tenant._id, permissionQuery, function (error, hasPermission) {
        if (error) {
          res.statusCode = 500;
          return res.json(error);
        }
        ...
        courseContent.retrieve({ _id: courseId }, {}, (err, results) => {
          self.transferSingleCourse(courseContent, results[0], toUserId, function (error) {
            // TODO error handling
            if (error) {
              res.statusCode = 500;
              return res.json(error);
    ...
    rest.post('/transfer_all_courses_ownership/from_user/:from_user_id/to_user/:to_user_id', function (req, res, next) {
      var fromUserId = req.params.from_user_id;
      var toUserId = req.params.to_user_id;
      var currentUser = self.getCurrentUser();

      self.retrieveUser({ _id: fromUserId }, function (error, user) {
        var resourceStr = permissions.buildResourceString(user._tenantId, 'transfer_all_courses_ownership');
        permissions.hasPermission(currentUser._id, 'create', resourceStr, function (error, hasPermission) {
          if (error) {
            res.statusCode = 500;
            return res.json(error);
          }
    ...
    rest.post('/user', function (req, res, next) {
      ...
      if (userData.password) {
        if (self.validatePassword(userData.password).length > 0) {
          return res.status(500).json('Make your password stronger with more capital letters, more numbers and special characters');
        }
