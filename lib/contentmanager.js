...
ContentPlugin.prototype.hasSchemaExtensions = function () {
  return false;
};
...
ContentPlugin.prototype.getSchemaExtensionPath = function () {
  var type = this.getModelName();
  var hasExtensions = this.hasSchemaExtensions();

  return hasExtensions ? path.join(configuration.serverRoot, 'plugins', 'content', type, 'extension.schema') : false;
};
...
ContentPlugin.prototype.onDatabaseCreated = function (db, next) {
  ...
  var schemaExtensionPath = this.getSchemaExtensionPath();
  var extensionSchema = {};
  var self = this;
  if (!schemaPath) {
    return next(null);
  ...
  if (schemaExtensionPath) {
    var schemaExtensionResults = fs.readFileSync(schemaExtensionPath);
    extensionSchema = schemaExtensionResults ? JSON.parse(schemaExtensionResults) : false;
  }
  ...
  fs.readFile(schemaPath, function (err, data) {
    ...
    try {
      ...
      if (extensionSchema && Object.keys(extensionSchema).length > 0) {
        schema.properties = _.defaults(schema.properties, extensionSchema.properties);
      }

      var originalSchema = extensionSchema && Object.keys(extensionSchema).length > 0 ? schema : JSON.parse(data);
...
ContentPlugin.prototype.create = function (data, next) {
  ...
  database.getDatabase(function (error, db) {
    ...
    data._type = data._type || self.getModelName();

    var permissionQuery = _.clone(data);
    if(!data._id && !data._courseId){
      const resource = permissions.buildResourceString(user.tenant._id, `/api/content/${permissionQuery._type}/own`);
      permissions.hasPermission(user._id, 'create', resource, function (error, isAllowed) {
        if (!isAllowed) {
          return next(new ContentPermissionError());
        }
      ...
      helpers.hasCoursePermission('create', user._id, user.tenant._id, permissionQuery, function (err, isAllowed) {
        if (!isAllowed) {
          return next(new ContentPermissionError());
        }
        return db.create(self.getModelName(), data, next);
  ...
  database.getDatabase(function (error, db) {
    ...
    db.retrieve(self.getModelName(), search, options, function (err, records) {
      ...
      async.each(records, function(contentItem, callback) {
        var permissionQuery = _.clone(contentItem);
        permissionQuery._type = permissionQuery._type || self.getModelName();
        helpers.hasCoursePermission('read', user._id, tenantId, permissionQuery, function (err, isAllowed) {
          if (!isAllowed) {
...
ContentPlugin.prototype.update = function (search, delta, next) {
  ...
  var permissionQuery = _.clone(delta);
  ...
  if (!permissionQuery.hasOwnProperty('_id') && search.hasOwnProperty('_id')) {
    permissionQuery._id = search._id;
  }
  ...
  helpers.hasCoursePermission('update', user._id, user.tenant._id, permissionQuery, function (err, isAllowed) {
    if (!isAllowed) {
      return next('You are not permitted to do that');
    }
...
ContentManager.prototype.setupRoutes = function () {
  ...
  rest.get('/clipboard/me', function (req, res, next) {
    var user = app.usermanager.getCurrentUser();
    ...
    database.getDatabase(function (error, db) {
      ...
      db.retrieve('clipboard', { createdBy: user._id }, function (error, results) {
        if (error) {
  ...
  rest.put('/content/:type/:id', function (req, res, next) {
    ...
    that.update(type, { _id: id }, val, function (error, updatedDocument) {
      if (error) {
        res.statusCode = error instanceof ContentTypeError ? 400 : 500;
        return res.json({ success: false, message: (typeof error === 'string' ? error : error.message) });
      }
  ...
  rest.delete('/content/:type/:id', function (req, res, next) {
    ...
    if (type === 'course') {
      helpers.hasOwnerPermission('delete', user._id, tenantId, {_id: id, _type: type}, function (err, isAllowed) {
        if (!isAllowed) {
          return res.status(403).json({success: false, message: (new ContentPermissionError()).message});
        }
        deleteContent();
      });
    }
