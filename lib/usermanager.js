  ...
  init: function (app) {
    ...
    rest.post('/user/resetpassword', function (req, res, next) {
      ...
      if (delta.password) {
        ...
        if (blocklist.includes(delta.password)) {
          return res.status(500).json('This password is too common, please use a more unique password');
        }
      }
      ...
      self.hasPermission('update', user, '/api/user/resetpassword', function (err, isAllowed) {
        ...
        logger.log('info', 'user password: ' + delta.password);
        logger.log('info', 'blocklist: ' + blocklist);
        logger.log('info', 'blocklist includes user password: ' + blocklist.includes(delta.password));
    ...
    rest.put('/user/me', function (req, res, next) {
      ...
      if (delta._isNewPassword) {
        ...
        if (confirmPassword !== delta.password) {
          return res.status(500).json('Confirmation password does not match password');
        }
    ...
    rest.put('/user/:id', function (req, res, next) {
      ...
      if (delta.confirmPassword && delta.confirmPassword !== delta.password) {
        return res.status(500).json('Confirmation password does not match password');
      }
