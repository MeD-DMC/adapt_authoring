...
exports = module.exports = {
  ...
  clearOtherSessions: function(req){
    database.getDatabase(function (err, db) {
      db.retrieve('session', {"session.passport.user._id": ObjectId(req.session.passport.user._id), "_id": { $ne: req.sessionID }}, { jsonOnly: true }, function (err, results){
        if (err) {
          logger.log('error', err);
        }
        if (!results) {
          logger.log('error', 'No other sessions to clear');
        } else {
          results.forEach(function(document){
            db.destroy('session', { _id: document._id }, function (error) {
              if(error){
                logger.log('error', 'An error occured while destroying a session');
              }
            });
          });
        }
