...
exports = module.exports = {
  ...
  validatePassword: function(password, user, next) {
    ...
    this.getUserDetails(user, function(err, result) {
      ...
      if (result && result.length > 0) {
        ...
        var verifyPasswordMatch = function(passwords) {
          if (passwords.length < 1) {
            return next(errors);
          }

          auth.validatePassword(password, passwords[passwords.length - 1], function(err, isMatch) {
            if (err) {
              errors.unshift(err);
            }
            if (isMatch) {
        ...
        if (existingUserPasswords.length < 1) {
          ...
          verifyPasswordMatch(existingUserPasswords);
        }
  ...
  init: function (app) {
    ...
    rest.post('/user/resetpassword', function (req, res, next) {
      ...
      if (delta.password) {
        self.validatePassword(delta.password, delta, function (result) {
          if (result.length > 0) {
          ...
          self.hasPermission('update', user, '/api/user/resetpassword', function (err, isAllowed) {
            ...
            auth.hashPassword(delta.password, function (err, hash) {
              ...
              self.getUserDetails(delta, function(err, result) {
                if (err) {
                  res.statusCode = 500;
                  return res.json(err);
                }
