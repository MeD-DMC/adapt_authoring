...
ContentPlugin.prototype.onDatabaseCreated = function (db, next) {
  ...
  fs.readFile(schemaPath, function (err, data) {
    ...
    try {
      ...
      if (modelName === 'block') {
        schema.properties['componentWidthRatio'] = {
          "type": "number",
          "default": 50
        }
      }
      ...
      db.addModel(modelName, schema, function (err, fullSchema) {
        ...
        if (err) {
          ...
          if (modelName === 'block' && originalSchema.properties) {
            originalSchema.properties['componentWidthRatio'] = {
              type: 'Range',
              inputType: 'Range',
              title: 'Component Width Ratio'
            }
          }
