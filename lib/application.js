...
Origin.prototype.createServer = function (options, cb) {
  ...
  */
  var app = this;
  const BUILDNUMBER = app.configuration.getConfig('buildPrefix') + app.configuration.getConfig('buildNumber');
  require('./database').getDatabase(function (err, db) {
    ...
    server.use(bodyParser.urlencoded({ limit: '50mb', extended: true }));
    const sessionKey = app.configuration.getConfig('devEnv') ? `connect-${app.configuration.getConfig('devEnv')}.sid` : 'connect.sid'
    server.use(session({
      key: sessionKey,
    ...
    //Adds middleware to include build number in header cookies
    server.use(function(req, res, next) {
      var options = {
        httpOnly: false
      }
      res.cookie('buildNumber', BUILDNUMBER, options);
      next();
    });
...
Origin.prototype.startServer = function (options) {
  ...
  checkPrerequisites(options, function (err, result) {
    ...
    app.createServer(serverOptions, function (error, server) {
      ...
      // use env port or default port if configuration or env is not available
      if (process.env.PORT) {
      ...
      const shutdownManager = new GracefulShutdownManager(app._httpServer);

      process.on('SIGINT', () => {
        logger.log('info', 'Server has received shutdown signal, gracefully terminating server');
        shutdownManager.terminate(() => {
          logger.log('info', 'Server has gracefully terminated');
          process.exit(0);
        });
      });
...
Origin.prototype.run = function (options) {
  ...
  this.preloadModule('messagemanager', require('./messagemanager'));
  this.preloadModule('helpdialogmanager', require('./helpdialogmanager'));
  //this.preloadModule('updatestructure', require('./updatestructure'));
  this.preload();
};
