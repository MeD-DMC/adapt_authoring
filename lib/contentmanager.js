...
ContentPlugin.prototype.create = function (data, next) {
  ...
  database.getDatabase(function (error, db) {
    ...
    if(!data._id && !data._courseId){
      const resource = permissions.buildResourceString(user.tenant._id, `/api/content/${data._type}/own`);
      ...
      helpers.hasCoursePermission('create', user._id, user.tenant._id, data, function (err, isAllowed) {
        if (!isAllowed) {
          return next(new ContentPermissionError());
        }
        return db.create(self.getModelName(), data, next);
  ...
  database.getDatabase(function (error, db) {
    ...
    db.retrieve(self.getModelName(), search, options, function (err, records) {
      ...
      async.each(records, function(contentItem, callback) {
          contentItem._type = contentItem._type || self.getModelName();
          helpers.hasCoursePermission('read', user._id, tenantId, contentItem, function (err, isAllowed) {
            if (!isAllowed) {
...
ContentPlugin.prototype.update = function (search, delta, next) {
  ...
  delta._type = delta._type || this.getModelName();
  ...
  helpers.hasCoursePermission('update', user._id, user.tenant._id, delta, function (err, isAllowed) {
    if (!isAllowed) {
      return next(new ContentPermissionError());
    }
...
ContentPlugin.prototype.destroy = function (search, force, next) {
  ...
  search._type = search._type || this.getModelName();
