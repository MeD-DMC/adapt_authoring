...
exports = module.exports = {
  ...
  retrieveAsset: function (search, options, next) {
    ...
    database.getDatabase(function (error, db) {
      ...
      __this.hasPermission('delete', user._id, user.tenant._id, '*', function(error, isAllowed) {
        ...
        //ESDC CUSTOM
        var checkOperators = options.operators
        if (checkOperators === undefined){
          db.retrieve('asset', search, options, next);
        }
        if (checkOperators !== undefined){
          var userId = user._id.toString();
          var tenantId = user.tenant._id.toString();
          var resource = permissions.buildResourceString(tenantId, '/api/hidden/asset');
          permissions.hasPermission(userId, 'read', resource, function(error, hasPermission){
            if (error) {
              return next(error);
            }
            var query = hasPermission ? { _isDeleted: false } : { _isDeleted: false, $nor: [ { hideAsset: true, createdBy: { $ne: userId } } ] };
            search = _.extend(search, query);
            db.retrieve('asset', search, options, next);
          });
        }
      });
  ...
  updateAsset: function (search, delta, next) {
    ...
    self.hasPermission('update', user._id, user.tenant._id, search, function (err, isAllowed) {
      ...
      database.getDatabase(function (error, db) {
        ...
        db.retrieve('asset', search, {}, function (error, assetRecs) {
          if (error) {
            return next(error);
          }
          if (assetRecs.length > 0 && delta.hideAsset && user._id != assetRecs[0].createdBy) {
            return next(new AssetPermissionError('You cannot hide an asset that does not belong to you', assetRecs[0]._id));
          }
        });
  ...
  postAsset: function (req, res, next) {
    ...
    const maxFileSize = configuration.getConfig('maxFileUploadSize');
    const customTypesData = configuration.getConfig('mimeTypesCustomUploadSize');
    var self = this;
    
    form.parse(req, function (error, fields, files) {
      function findValueByKey(obj, key) {
        var mimekey = key.split('/')[0];
        return obj[mimekey] || null; // Return the value if found, otherwise return null
      }

      var maxUpload = bytes(findValueByKey(customTypesData, form.openedFiles[0].type));
      if (maxUpload === null){
        maxUpload = maxFileSize;
      }
      if (form.bytesExpected > maxUpload){
          return _respondError(app.polyglot.t('app.uploadsizeerror', {
          max: bytes.format(maxUpload),
  ...
  queryAssets: function (req, res, next) {
    ...
    search = _.mapObject(search, function (value, key) {
    ...
    async.each(
      ...
      function (key, nextKey) {
        ...
        if ('string' === typeof search[key] && key !== 'createdBy') {
