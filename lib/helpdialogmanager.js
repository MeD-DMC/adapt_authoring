...
exports = module.exports = {
  ...
  init: function (app) {
    ...
    // todo remove the underscore and just merge the words as a whole
    rest.get('/help_dialogs', function (req, res, next) {
      var masterHelpDialogs = self.getMasterHelpDialogs();

      if (masterHelpDialogs) {
        // todo query by name instead of properties.name
        self.retrieveHelpDialogs({'properties.name': masterHelpDialogs}, function (err, results) {
          if (err) {
            res.statusCode = 500;
            return res.json(err);
          }
    ...
    rest.put('/help_dialogs', function (req, res, next) {
      ...
      self.updateHelpDialogs({'properties.name': help_dialogs}, delta, function(err) {
        if (err) {
          return next(err);
        }
